...
export default class ExcelMapper {
    static mapDatamasterSatuan(data, faskesUuid) {
        data.forEach((item) => {
            if (!item['Kode Satuan*'] || !item['Nama Satuan*'] || !item['Satuan Dosis*']) {
            ...
            item.status = true;
            item.satuan_dosis = item['Satuan Dosis*'] === "Aktif" ? true : item['Satuan Dosis*'] === "aktif";
    ...
    static mapDatamasterLokasiStok(data, faskesUuid) {
        ...
        data.forEach((item) => {
            if (!item['Kode Lokasi Stok*'] || !item['Nama Lokasi Stok*'] || !item['Jenis Lokasi*']) {
                throw new BadRequestException("data tidak lengkap atau kunci salah");
            }
    ...
    static mapDatamasterAturanPakai(data, faksesUuid) {
        data.forEach((item) => {
            if (!item['Kode Aturan Pakai*'] || !item['Nama Aturan Pakai*'] || !item['Periode Unit*'] || !item['Frekuensi*'] || !item['Periode*']) {
                throw new BadRequestException("data tidak lengkap atau kunci salah");
            }
    ...
    static mapDatamasterItemMedis(data, faskesUuid){
        data.forEach((item) => {
            if (!item['Kode Item Medis*'] || !item['Nama Item Medis*'] || !item['Jenis Item*'] || !item['Satuan Penggunaan*'] || !item['Bentuk Sediaan*'] || !item['Dosis Kemasan*'] || !item['Satuan Dosis*'] || !item['Kategori Item*']) {
                throw new BadRequestException("data tidak lengkap atau kunci salah");
            }
            ...
            item.satuan_penggunaan_code = (item['Satuan Penggunaan*'] || "").toUpperCase();
            ...
            item.ingridients = (item['Komposisi'] || "").replace(/\s+/g, '').toUpperCase().split(',').map(item => ({ name: item }));

            item.jenis_stok_codes = (item['Jenis Stok'] || "").replace(/\s+/g, '').toUpperCase().split(',');
    ...
    static mapDatamasterConversion(data, faskesUuid){
        data.forEach((item) => {
            if (!item['Kode Item Medis*'] || !item['Satuan Pembelian*'] || !item['Satuan Penggunaan*'] || !item['Konversi*']) {
                throw new BadRequestException("data tidak lengkap atau kunci salah");
            }
